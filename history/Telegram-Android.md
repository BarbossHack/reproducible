# Telegram Android - Build History

Reproducible build instructions: <https://core.telegram.org/reproducible-builds#reproducible-builds-for-android>

<table align="center">
    <thead>
        <tr>
            <th width="10%">Version</th>
            <th width="15%">Release Date</th>
            <th width="25%">Reproducible</th>
            <th width="10%">Workflow</th>
            <th width="40%">Notes</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>v12.4.1</td>
            <td>2026-02-09</td>
            <td>✅ Yes <i>(PlayStore)</i><br>✅ Yes <i>(Website)</i></td>
            <td><a href="https://github.com/BarbossHack/reproducible/actions/runs/22824584053">#22824584053</a></td>
            <td>Reproducible builds script provided by Telegram does not allow verification of the PlayStore and Website APK. The dex files are not in the same order (although they are striclty identical) and the PlayStore APK miss the `baseline.prof` file. Reported to @BotSupport. The file `version-control-info.textproto` also differs (see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>).</td>
        </tr>
        <tr>
            <td>v12.4.0</td>
            <td>2026-02-06</td>
            <td>✅ Yes <i>(PlayStore)</i><br>✅ Yes <i>(Website)</i></td>
            <td><a href="https://github.com/BarbossHack/reproducible/actions/runs/22826249660">#22826249660</a></td>
            <td>Reproducible builds script provided by Telegram does not allow verification of the PlayStore and Website APK. The dex files are not in the same order (although they are striclty identical) and the PlayStore APK miss the `baseline.prof` file. Reported to @BotSupport. The file `version-control-info.textproto` also differs (see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>).</td>
        </tr>
        <tr>
            <td>v12.3.1</td>
            <td>2026-01-06</td>
            <td>✅ Yes <i>(PlayStore)</i><br>✅ Yes <i>(Website)</i></td>
            <td><a href="https://github.com/BarbossHack/reproducible/actions/runs/22826253462">#22826253462</a></td>
            <td>Reproducible builds script provided by Telegram does not allow verification of the PlayStore and Website APK. The dex files are not in the same order (although they are striclty identical) and the PlayStore APK miss the `baseline.prof` file. Reported to @BotSupport. The file `version-control-info.textproto` also differs (see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>).</td>
        </tr>
        <tr>
            <td>v12.3.0</td>
            <td>2026-01-03</td>
            <td>✅ Yes <i>(PlayStore)</i><br>✅ Yes <i>(Website)</i></td>
            <td><a href="https://github.com/BarbossHack/reproducible/actions/runs/22826254505">#22826254505</a></td>
            <td>Reproducible builds script provided by Telegram does not allow verification of the PlayStore and Website APK. The dex files are not in the same order (although they are striclty identical) and the PlayStore APK miss the `baseline.prof` file. Reported to @BotSupport. The file `version-control-info.textproto` also differs (see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>).</td>
        </tr>
        <tr>
            <td>v12.2.10</td>
            <td>2025-12-07</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.9</td>
            <td>2025-12-05</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.7</td>
            <td>2025-11-28</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.6</td>
            <td>2025-11-26</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.5</td>
            <td>2025-11-21</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.4</td>
            <td>2025-11-20</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.3</td>
            <td>2025-11-17</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v12.2.1</td>
            <td>2025-11-17</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
        <tr>
            <td>v11.4.1</td>
            <td>2025-08-04</td>
            <td>✅ Yes <i>(Website)</i></td>
            <td>-</td>
            <td>The file `version-control-info.textproto` differ, see <a href="https://github.com/DrKLO/Telegram/pull/1899">#1899</a>.</td>
        </tr>
    </tbody>
</table>
